...
class SyntheticSemanticHandoff(_SyntheticSemanticHandoffBase):
    ...
    def governance_startup_recovery_snapshot(self, representation_id):
        ...
        commit_cursor = getattr(
            self, "startup_commit_cursor", target_ordinal
        )
        return {
            "global_attempt_total": self.global_attempt_total,
            "global_unknown": self.global_unknown,
            "last_global_ordinal": self.global_attempt_total,
            "next_global_ordinal": self.global_attempt_total + 1,
            "absolute_cap": self.absolute_cap,
            ...
            "commit_cursor_ordinal": commit_cursor,
            "commit_cursor_fingerprint": "sha256:" + "1" * 64,
            "latest_attempt_receipt_fingerprint": "sha256:" + "2" * 64,
            "semantic_run_id": "semantic_run_" + "3" * 32,
            "batch_ordinal": 1,
            ...
            "ledger_tail_attempt_receipt_fingerprint": "sha256:" + "9" * 64,
        }
...
class WechatDigestTests(unittest.TestCase):
    ...
    def item_governance_startup_recovery_fixture(
        self, *, fail_restart: bool = False, atomic_count: int = 3
    ):
        self.create_object()
        self.semantic.provider.mode = "all_candidate"
        self.semantic.global_attempt_total = 372
        capture = SyntheticCaptureProvider([
            message(index, conversation="issue-194-current")
            for index in range(1, atomic_count + 1)
        ])
        provider = StartupFailOnceBatchProvider(fail_restart=fail_restart)
        service = WechatDigestService(
            workspace=self.workspace,
    ...
    def test_item_governance_startup_recovery_installs_zero_write_and_resumes_once(self) -> None:
        service, provider, run_id, item_id, atomic_ids = self.item_governance_startup_recovery_fixture()
        authority_ref = "https://github.com/leevi2010-cursor/ArcheOS/issues/194#issuecomment-5415298053"
        manifest = service.build_governance_startup_recovery_manifest(authority_ref=authority_ref)
        self.assertEqual(manifest["semantic_snapshot"]["last_global_ordinal"], 372)
        self.assertEqual(manifest["recovery_binding"]["ordered_atomic_information_ids"], atomic_ids)
        path = Path(self.temporary.name) / "issue-194-authority.json"
        ...
        receipt = service.resolve_governance_startup_failure(
            authority_ref=authority_ref, authority_manifest_file=path
        )
        self.assertEqual(status_path.read_bytes(), status_before)
        ...
        self.assertEqual(service.resolve_governance_startup_failure(
            authority_ref=authority_ref, authority_manifest_file=path
        ), receipt)
        result = service.run(max_terminal_items=1)
        self.assertTrue(result.segment_safe_stopped or result.checkpoint_published)
        self.assertEqual(provider.attempts, attempts + 1)
        self.assertEqual(self.semantic.provider.calls, semantic_calls)
        observed = service.run_store.status(run_id)["items"][item_id]
        self.assertIn(observed["state"], {"processed", "pending_human"})
    ...
    def test_item_governance_startup_recovery_sdk_preflight_is_before_status_write(self) -> None:
        ...
        authority_ref = "https://github.com/leevi2010-cursor/ArcheOS/issues/194#issuecomment-5415298053"
        manifest = service.build_governance_startup_recovery_manifest(authority_ref=authority_ref)
        path = Path(self.temporary.name) / "issue-194-sdk.json"
        path.write_text(json.dumps(manifest), encoding="utf-8")
        os.chmod(path, 0o600)
        service.resolve_governance_startup_failure(authority_ref=authority_ref, authority_manifest_file=path)
        status_path = service.run_store.runs_root / run_id / "status.json"
        before = status_path.read_bytes()
        business_before = self.governance_business_state(service)
        with patch("archeos.wechat_digest.importlib.metadata.version", return_value="0.144.3"), self.assertRaises(RuntimeError):
            service.run(max_terminal_items=1)
        self.assertEqual(status_path.read_bytes(), before)
        self.assertEqual(self.governance_business_state(service), business_before)
        self.assertEqual(provider.attempts, 1)

    def test_item_governance_startup_semantic_drift_is_before_status_write(self) -> None:
        ...
        path = Path(self.temporary.name) / "issue-194-semantic-drift.json"
        path.write_text(json.dumps(manifest), encoding="utf-8")
        os.chmod(path, 0o600)
        service.resolve_governance_startup_failure(
            authority_ref=authority_ref,
    ...
    def test_item_governance_startup_recovery_rejects_drift_before_receipt(self) -> None:
        service, _provider, run_id, item_id, _atomic_ids = self.item_governance_startup_recovery_fixture(atomic_count=2)
        authority_ref = "https://github.com/leevi2010-cursor/ArcheOS/issues/194#issuecomment-5415298053"
        manifest = service.build_governance_startup_recovery_manifest(authority_ref=authority_ref)
        self.assertEqual(manifest["recovery_binding"]["candidate_count"], 2)
        path = Path(self.temporary.name) / "issue-194-drift.json"
        path.write_text(json.dumps(manifest), encoding="utf-8")
        os.chmod(path, 0o600)
        status = service.run_store.status(run_id)
        status["items"][item_id]["context_object_ids"] = ["obj_" + "9" * 32]
        service.run_store.update_status(run_id, status)
        with self.assertRaises(WechatDigestError):
            service.resolve_governance_startup_failure(authority_ref=authority_ref, authority_manifest_file=path)
        self.assertEqual(service.run_store.governance_startup_recoveries(run_id), ())

    def test_item_governance_startup_snapshot_preserves_uncommitted_ledger_tail(self) -> None:
        service, _provider, run_id, _item_id, _atomic_ids = (
            self.item_governance_startup_recovery_fixture()
        )
        self.semantic.global_attempt_total = 373
        self.semantic.startup_target_ordinal = 372
        ...
        semantic = manifest["semantic_snapshot"]
        self.assertEqual(semantic["global_attempt_total"], 373)
        self.assertEqual(semantic["last_global_ordinal"], 373)
        self.assertEqual(semantic["next_global_ordinal"], 374)
        self.assertEqual(semantic["target_global_ordinal_range"], [372, 372])
        self.assertEqual(semantic["commit_cursor_ordinal"], 372)
        self.assertEqual(service.run_store.status(run_id), before)

    def test_item_governance_startup_allows_unrelated_v2_history(self) -> None:
        service, _provider, run_id, _item_id, atomic_ids = (
            self.item_governance_startup_recovery_fixture()
        )
        authority_ref = "https://github.com/leevi2010-cursor/ArcheOS/issues/194#issuecomment-5415298053"
        manifest = service.build_governance_startup_recovery_manifest(
        ...
        unrelated_binding = {
            **manifest["recovery_binding"],
            "item_id": "conversation:historical-item",
        }
        unrelated_without = {
            "schema_version": "wechat-governance-startup-recovery/1.0",
            "artifact_kind": "governance_startup_recovery",
            "authority_ref": "https://github.com/leevi2010-cursor/ArcheOS/issues/193#issuecomment-1",
            "authority_manifest_fingerprint": "sha256:" + "a" * 64,
            "authority_manifest_raw_fingerprint": "sha256:" + "b" * 64,
            "recovery_binding": unrelated_binding,
            "atomic_effect_bindings": manifest["atomic_effect_bindings"],
            "business_tree_fingerprint": manifest["business_tree_fingerprint"],
            "semantic_continuation_fingerprint": "sha256:" + "c" * 64,
        ...
        unrelated = {
            **unrelated_without,
            "receipt_fingerprint": _sha256_bytes(
                _canonical_json(unrelated_without).encode("utf-8")
            ),
        }
        service.run_store.publish_item_scoped_governance_startup_receipt(
            run_id, receipt=unrelated
        )
        retry_without = {
            "schema_version": "wechat-governance-startup-retry/1.0",
            "artifact_kind": "governance_startup_retry_consumption",
            "recovery_receipt_fingerprint": unrelated["receipt_fingerprint"],
            "run_id": run_id,
            "item_id": "conversation:historical-item",
            "atomic_information_fingerprint": _governance_atomic_fingerprint(
                atomic_ids
            ),
            "retry_attempt": 1,
        ...
        retry = {
            **retry_without,
            "receipt_fingerprint": _sha256_bytes(
                _canonical_json(retry_without).encode("utf-8")
            ),
        }
        service.run_store.publish_item_scoped_governance_startup_receipt(
            run_id, receipt=retry
        )
        rebuilt = service.build_governance_startup_recovery_manifest(
            authority_ref=authority_ref
        )
        self.assertEqual(rebuilt, manifest)

    def test_item_governance_startup_retry_failure_is_consumed(self) -> None:
        service, provider, run_id, _item_id, _atomic_ids = self.item_governance_startup_recovery_fixture(fail_restart=True)
        authority_ref = "https://github.com/leevi2010-cursor/ArcheOS/issues/194#issuecomment-5415298053"
        manifest = service.build_governance_startup_recovery_manifest(authority_ref=authority_ref)
        path = Path(self.temporary.name) / "issue-194-consumed.json"
        path.write_text(json.dumps(manifest), encoding="utf-8")
        os.chmod(path, 0o600)
        service.resolve_governance_startup_failure(authority_ref=authority_ref, authority_manifest_file=path)
        with self.assertRaises(WechatDigestError):
        ...
        attempts = provider.attempts
        self.assertEqual(len(service.run_store.governance_startup_retries(run_id)), 1)
        with self.assertRaisesRegex(WechatDigestError, "机会已消费"):
            service.run(max_terminal_items=1)
        self.assertEqual(provider.attempts, attempts)

    def test_openai_codex_sdk_preflight_checks_version_before_import(self) -> None:
        with patch("archeos.wechat_digest.importlib.metadata.version", return_value="0.144.3"), patch("archeos.wechat_digest.importlib.import_module") as importer:
            with self.assertRaises(RuntimeError):
                _require_openai_codex_sdk()
            importer.assert_not_called()
        with patch("archeos.wechat_digest.importlib.metadata.version", return_value="0.144.4"), patch("archeos.wechat_digest.importlib.import_module") as importer:
            _require_openai_codex_sdk()
    ...
    def test_production_shaped_service_max3_uses_one_capture_and_serial_governance(
        ...
        self.assertEqual(sum(item.capture_attempts for item in results), 2)
        self.assertEqual(sum(item.capture_successes for item in results), 2)
        self.assertEqual(
            tuple(
                reason
            ...
            ("upper_bound_probe", "initial_history_window"),
        )
    ...
    def test_capture_failure_keeps_attempt_observable_without_success(self) -> None:
        class FailingCaptureProvider(SyntheticCaptureProvider):
            def capture(self, *args, **kwargs):
                del args, kwargs
                raise RuntimeError("synthetic capture failure")

        service = self.service(FailingCaptureProvider([]))
        with patch(
            "archeos.wechat_digest._require_openai_codex_sdk"
        ), self.assertRaisesRegex(WechatDigestError, "checkpoint 未推进"):
            service.run(since="2023-01-01")
        self.assertEqual(service._segment_performance["capture_attempts"], 1)
        self.assertEqual(service._segment_performance["capture_successes"], 0)
        self.assertEqual(
            service._segment_performance["capture_provider_calls"], 0
        )
        self.assertEqual(service._capture_reasons, ["since_window"])
        self.assertIsNone(service.run_store.checkpoint())
        self.assertIsNone(service.run_store.active_run_id())
        ...
        for window_count in (1, 5, 10):
            with self.subTest(windows=window_count):
                ...
                with patch.object(
                    ...
                    "get",
                    side_effect=AssertionError(
                        "completed history must not read Source"
                    ),
                ), patch.object(
                    service.representation_repository,
                    "get",
                    side_effect=AssertionError(
                        "completed history must not read Representation"
                    ),
                ), patch.object(
                    service.information_store,
                    "get_current",
                    side_effect=AssertionError(
                        "completed history must not read Atomic Information"
                    ),
                ), patch.object(
                    service,
                    "_verify_semantic_receipts",
                    side_effect=AssertionError(
                        "completed history must not read semantic package"
                    ),
                ):
                ...
                with patch.object(
                    service,
                    "_load_completed_window_lightweight",
                    side_effect=AssertionError(
                        "completed chain must be cached within one CLI run"
                    ),
                ):
                    cached = service._semantic_authority_binding(
                        str(plan["run_id"])
                    )
                self.assertEqual(cached, binding)

    def test_completed_window_lightweight_drift_fails_before_any_side_effect(
        self,
    ) -> None:
        for drift in ("plan", "status", "checkpoint", "summary", "receipt"):
            with self.subTest(drift=drift):
                root = (
                    self.workspace
                    / "02_processing"
                    / f"wechat_digest_completed_drift_{drift}"
                )
                run_store = WechatDigestRunStore(root)
                messages = [
                    message(1, timestamp=1_700_000_000),
                    message(2, timestamp=1_700_000_001),
                ]
                global_upper = messages[-1].cursor
                run_ids = []
                after = ZERO_CURSOR
                for index, captured_message in enumerate(messages):
                    ...
                    capture_receipt = run_store.publish_capture_artifacts(
                        str(legacy_plan["run_id"]),
                        capture,
                        plan_binding_fingerprint=_plan_fingerprint(legacy_plan),
                        capture_ms=1,
                    ...
                    if index == 0:
                        ...
                        for item in status["items"].values():
                            item["state"] = "processed"
                            item["representation_id"] = "rep_" + "1" * 32
                            item["atomic_information_ids"] = [
                                "info_" + "1" * 32
                            ]
                    run_store.create(plan, status)
                    if index == 0:
                        run_store.clear_active()
                    run_ids.append(str(plan["run_id"]))
                    after = captured_message.cursor
                completed_run_id, active_run_id = run_ids
                run_store.publish_checkpoint(
                    completed_run_id, messages[0].cursor
                )

                completed_dir = run_store.runs_root / completed_run_id
                if drift == "plan":
                    path = completed_dir / "plan.json"
                    value = json.loads(path.read_text(encoding="utf-8"))
                    value["conversations"][0]["content_hash"] = (
                        "sha256:" + "0" * 64
                    )
                elif drift == "status":
                    path = completed_dir / "status.json"
                    value = json.loads(path.read_text(encoding="utf-8"))
                    value["plan_fingerprint"] = "sha256:" + "0" * 64
                elif drift == "checkpoint":
                    path = run_store.checkpoint_path
                    value = json.loads(path.read_text(encoding="utf-8"))
                    value["run_id"] = "run_" + "0" * 32
                elif drift == "summary":
                    path = completed_dir / "capture" / "summary.json"
                    value = json.loads(path.read_text(encoding="utf-8"))
                    value["message_count"] += 1
                else:
                    path = completed_dir / "capture" / "receipt.json"
                    value = json.loads(path.read_text(encoding="utf-8"))
                    value["message_count"] += 1
                path.write_text(json.dumps(value), encoding="utf-8")
                ...
                with patch.object(
                    capture_provider,
                    "capture",
                    side_effect=AssertionError("connector must remain unused"),
                ), patch.object(
                    run_store,
                    "load_capture_artifacts",
                    side_effect=AssertionError(
                    ...
                    with self.assertRaises(WechatDigestError):
                        service._semantic_authority_binding(active_run_id)
                after_files = {
                    str(candidate.relative_to(root)): candidate.read_bytes()
                    for candidate in root.rglob("*")
                    if candidate.is_file()
...
class WechatCaptureHelperTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        with closing(sqlite3.connect(self.database)) as connection:
            ...
            connection.execute(
                f"CREATE INDEX [{self.table_name}_create_time] "
                f"ON [{self.table_name}] (create_time, local_id)"
            )
            connection.commit()
    ...
    @contextmanager
    def capture_runtime(self, sessions=None):
    ...
    def _legacy_capture(self, request, *, sessions=None):
        ...
        def unbounded_window_discovery(
            located_tables,
            *,
            after,
            window_days,
            ...
            del window_days, message_limit
            rows = _all_cursor_rows(
                located_tables,
                start_timestamp=after[0],
                end_timestamp=(
                    None if end_cursor is None else end_cursor[0]
                ),
            )
            return rows, len(rows)

        with self.capture_runtime(sessions), patch.object(
            wechat_capture_helper,
            "_all_cursor_rows",
            ...
            wechat_capture_helper,
            "_bounded_window_cursor_rows",
            side_effect=unbounded_window_discovery,
        ):
    ...
    def test_fixed_upper_is_sql_bounded_and_byte_equivalent(self) -> None:
        ...
        self.assertEqual(optimized["observed_upper"], legacy["observed_upper"])
        self.assertEqual(optimized["messages"], legacy["messages"])
        self.assertEqual(
            optimized["metrics"]["materialized_cursor_rows"], 4
        )
        returned = [
            (
    ...
    def test_all_history_upper_is_sql_bounded_and_byte_equivalent(self) -> None:
        ...
        with self.capture_runtime(), patch.object(
            wechat_capture_helper,
            "_all_cursor_rows",
            side_effect=AssertionError("history window must not scan full tail"),
        ) as full_tail, patch.object(
            wechat_capture_helper,
            "_bounded_window_cursor_rows",
            wraps=_bounded_window_cursor_rows,
        ) as discovery:
            optimized = capture_with_wechat_cli(request)
        full_tail.assert_not_called()
        self.assertEqual(discovery.call_args.kwargs["end_cursor"], upper)
        self.assertEqual(optimized["observed_upper"], legacy["observed_upper"])
        self.assertEqual(optimized["messages"], legacy["messages"])
    ...
    def test_production_shaped_probe_reads_only_table_maxima(self) -> None:
        ...
        with patch.dict(sys.modules, self.modules), patch.object(
            ...
            wechat_capture_helper,
            "_all_cursor_rows",
            side_effect=AssertionError("new window must not scan full tail"),
        ) as full_scan, patch.object(
            wechat_capture_helper,
            "_bounded_window_cursor_rows",
            wraps=_bounded_window_cursor_rows,
        ) as bounded_discovery:
            for _ in range(7):
        ...
        self.assertEqual(bounded_discovery.call_count, 7)
    ...
    def test_new_window_discovery_is_bounded_and_semantically_unchanged(self) -> None:
        ...
        self.assertLessEqual(
            result["metrics"]["materialized_cursor_rows"], 5
        )

    def test_multi_database_ties_and_thirty_day_boundary_match_legacy(self) -> None:
        second_database = self.root / "msg-second.db"
        second_database_key = "msg/msg-second.db"
        second_username = "wxid_synthetic_second"
        second_table = "Msg_" + hashlib.md5(
            second_username.encode()
        ).hexdigest()
        exact_thirty_day_boundary = 100 + 30 * 24 * 60 * 60
        with closing(sqlite3.connect(second_database)) as connection:
            ...
            connection.executemany(
                ...
                [
                    (1, 1, 200, "sender", "second-1", 0),
                    (2, 1, 200, "sender", "second-2", 0),
                    (3, 1, 200, "sender", "second-3", 0),
                    (
                        4,
                        1,
                        exact_thirty_day_boundary,
                        "sender",
                        "excluded-boundary",
                        0,
                    ),
            ...
            connection.execute(
                f"CREATE INDEX [{second_table}_create_time] "
                f"ON [{second_table}] (create_time, local_id)"
            )
            connection.commit()
        self.app.cache[second_database_key] = str(second_database)
        self.app.msg_db_keys = (self.database_key, second_database_key)
        sessions = (
            (self.username, "Synthetic Conversation", False),
            (second_username, "Synthetic Conversation 2", False),
        )
        request = self.request(message_limit=4)
        legacy = self._legacy_capture(request, sessions=sessions)
        with self.capture_runtime(sessions), patch.object(
            wechat_capture_helper,
            "_all_cursor_rows",
            side_effect=AssertionError("new window must not scan full tail"),
        ):
            bounded = capture_with_wechat_cli(request)
        self.assertEqual(bounded["observed_upper"], legacy["observed_upper"])
        self.assertEqual(bounded["messages"], legacy["messages"])
        self.assertEqual(len(bounded["messages"]), 4)
        self.assertTrue(
            all(
                item["timestamp"] < exact_thirty_day_boundary
                for item in bounded["messages"]
            )
        )

    def test_after_cursor_inside_hash_order_ties_matches_legacy_across_limit(
        self,
    ) -> None:
        second_database = self.root / "msg-cursor-suffix.db"
        second_database_key = "msg/msg-cursor-suffix.db"
        second_username = "wxid_cursor_suffix_second"
        second_table = "Msg_" + hashlib.md5(
            second_username.encode()
        ).hexdigest()
        with closing(sqlite3.connect(second_database)) as connection:
            ...
            connection.executemany(
                f"INSERT INTO [{second_table}] VALUES (?, ?, ?, ?, ?, ?)",
                [
                    (1, 1, 200, "sender", "second-tie-1", 0),
                    (2, 1, 200, "sender", "second-tie-2", 0),
                    (3, 1, 200, "sender", "second-tie-3", 0),
                    (4, 1, 200, "sender", "second-tie-4", 0),
                    (5, 1, 250, "sender", "second-later", 0),
                ],
            )
            connection.execute(
                f"CREATE INDEX [{second_table}_create_time] "
        ...
        tie_cursors = [
            self.cursors[local_id] for local_id in (2, 3, 4)
        ] + [
            (
                200,
                capture_digest("wechat_conversation", second_username),
                capture_digest(
                    "wechat_message",
                    second_username,
                    second_database_key,
            ...
            for local_id in (1, 2, 3, 4)
        ]
        local_id_message_keys = [
            capture_digest(
                "wechat_message",
                second_username,
                second_database_key,
        ...
        self.assertNotEqual(local_id_message_keys, sorted(local_id_message_keys))
        ordered_ties = sorted(tie_cursors)
        after = ordered_ties[-3]
        expected_suffix = ordered_ties[-2:]
        request = self.request(
            after_cursor=self.cursor_dict(after),
            message_limit=3,
        )
        legacy = self._legacy_capture(request, sessions=sessions)
        with self.capture_runtime(sessions), patch.object(
        ...
        legacy_projection = {
            key: legacy[key]
            for key in ("schema_version", "observed_upper", "messages")
        }
        ...
        self.assertEqual(
            json.dumps(optimized_projection, sort_keys=True, separators=(",", ":")),
            json.dumps(legacy_projection, sort_keys=True, separators=(",", ":")),
        )
        returned = [
            (
                item["cursor"]["timestamp"],
        ...
        self.assertEqual(returned[-1][0], 250)

    def test_materialized_metric_counts_sql_boundary_rows_including_reread(
        self,
    ) -> None:
        database = self.root / "large-boundary-tie.db"
        username = "wxid_large_boundary_tie"
        table_name = "Msg_" + hashlib.md5(username.encode()).hexdigest()
        with closing(sqlite3.connect(database)) as connection:
            connection.execute(
                f"CREATE TABLE [{table_name}] ("
                "local_id INTEGER PRIMARY KEY, create_time INTEGER)"
            )
            connection.executemany(
                f"INSERT INTO [{table_name}] VALUES (?, ?)",
                [(1, 100)] + [(index, 200) for index in range(2, 52)],
            )
            connection.execute(
                f"CREATE INDEX [{table_name}_create_time] "
                f"ON [{table_name}] (create_time, local_id)"
            )
            connection.commit()
        located = {
            str(database): ((
                "msg/large-boundary-tie.db",
                username,
                "Synthetic Boundary",
                False,
                table_name,
        ...
        rows, sql_cursor_rows = _bounded_window_cursor_rows(
            located,
            after=(99, "", ""),
            window_days=30,
            message_limit=3,
        ...
        self.assertEqual(sql_cursor_rows, 3 + 50)
        self.assertEqual(sql_cursor_rows, 1 * 3 + 50)

    def test_tail_1x_5x_10x_keeps_materialized_rows_bounded(self) -> None:
        materialized_counts = []
        legacy_counts = []
        bounded_wall_ns = []
        for tail_factor in (1, 5, 10):
            database = self.root / f"tail-{tail_factor}.db"
            username = f"wxid_tail_{tail_factor}"
            table_name = "Msg_" + hashlib.md5(username.encode()).hexdigest()
            with closing(sqlite3.connect(database)) as connection:
                connection.execute(
                    f"CREATE TABLE [{table_name}] ("
                ...
                first_window = [
                    (index + 1, 100 + index) for index in range(100)
                ]
                future_tail = [
                    (
                        101 + index,
                        100 + 31 * 24 * 60 * 60 + index,
                    )
                    for index in range(1000 * tail_factor)
                ]
                connection.executemany(
                    f"INSERT INTO [{table_name}] VALUES (?, ?)",
                    first_window + future_tail,
                )
                connection.execute(
                    f"CREATE INDEX [{table_name}_create_time] "
                    f"ON [{table_name}] (create_time, local_id)"
                ...
                query_plan = connection.execute(
                    f"EXPLAIN QUERY PLAN SELECT local_id, create_time "
                    f"FROM [{table_name}] WHERE create_time > ? "
                    "AND create_time < ? "
                    "ORDER BY create_time ASC, local_id ASC LIMIT ?",
                    (99, 100 + 30 * 24 * 60 * 60, 100),
                ).fetchall()
                connection.commit()
            self.assertTrue(
                any("INDEX" in str(row[-1]).upper() for row in query_plan)
            )
            located = {
                str(database): ((
                    f"msg/tail-{tail_factor}.db",
                    username,
                    "Synthetic Tail",
                    False,
                    table_name,
            ...
            legacy = _all_cursor_rows(located, start_timestamp=99)
            started = time.monotonic_ns()
            bounded, materialized = _bounded_window_cursor_rows(
                located,
                after=(99, "", ""),
                window_days=30,
                message_limit=100,
            )
            bounded_wall_ns.append(time.monotonic_ns() - started)
            legacy_counts.append(len(legacy))
            materialized_counts.append(materialized)
            self.assertEqual(len(bounded), 100)

        self.assertEqual(legacy_counts, [1100, 5100, 10100])
        self.assertEqual(materialized_counts, [101, 101, 101])
        self.assertTrue(all(value >= 0 for value in bounded_wall_ns))
