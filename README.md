...
联系人处理默认把 Semantic、Governance 与联系人级归并纳入同一个 50 次模型调用上限；真实验收可用 `--authority-ref <批准链接> --provider-absolute-cap 50` 把批准来源和绝对上限写入私有运行记录。业务验收包与性能摘要会分别报告三类调用、总调用、恢复调用和剩余额度。
