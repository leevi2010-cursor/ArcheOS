...
9. **整体目标与 ROI 优先**：开发优先级同时考虑业务价值、验证速度、投入成本与可复用收益；优先交付能让用户检查和使用的最小端到端闭环，不以处理条数、测试数量、治理门数量或局部技术完整度冒充产品进展。
10. **风险投入与产品阶段匹配**：MVP 保留正常故障可恢复、业务数据不丢失、Source / Evidence 可追溯、隐私与凭证保护，以及外部或不可逆操作的人类授权边界；恶意攻击、主动篡改、全面安全加固和企业级合规只在真实风险、客户要求或正式生产门触发时进入主线。
