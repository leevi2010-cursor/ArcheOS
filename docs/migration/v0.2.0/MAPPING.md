...
| person / company / project / business-line base table | Object + approved Role | 不恢复互斥 base table；Role 必须经当前 vocabulary 与治理确认 |
| display label / historical name | Name assignment | 稳定身份使用 `object_id`；Name 变化保留历史 |
| legacy status / phase | Lifecycle（仅明确语义） | 不把任意状态字段直接升级为 Lifecycle；冲突或缺证时 human review |
| legacy relation / foreign key | Relationship（仅有 Evidence 时） | 不按旧外键自动造关系；方向、语义、端点与 Evidence 必须明确 |
...
- ambiguity、consequential structure、correction、merge、delete、split 保持 Human Judgment；
